  ...
  const handleDrop = async (e: React.DragEvent) => {
    ...
    try {
      ...
      if (approved.length === 0) {
        setDropError('No supported local video files were dropped.')
        return
      }
      onVideosSelected(approved)
    } catch (error) {
      setDropError(error instanceof Error ? error.message : 'Could not open the dropped video files.')
    }
  }
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', height: '100vh' }}>
      ...
      <div style={heroPanel}>
        ...
        <div style={{ position: 'relative', zIndex: 2 }}>
          ...
          <h1 style={{
            ...
            TENNIS <span style={{ color: '#cc4e0e' }}>COACH</span><span style={{ color: '#cc4e0e' }}>.</span>
          </h1>
      ...
      <div style={actionPanel}>
        <div style={{ position: 'relative', zIndex: 2, maxWidth: 360, WebkitAppRegion: 'no-drag' } as CSSProperties}>
          ...
          {dropError && <p style={{ color: 'var(--color-danger)', fontSize: 12, marginTop: 8 }}>{dropError}</p>}
