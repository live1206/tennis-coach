...
function AppInner() {
  ...
  const analyzeVideo = useCallback(async (video: VideoRecord, runId: number, options?: { reuseReport?: boolean }) => {
    ...
    let result
    try {
      result = await window.api.runAnalysis(video.path)
    } catch (error) {
      cleanup()
      if (!isCurrentBatchRun(runId)) return false
      dispatch({
        ...
        videoId: video.id,
        message: error instanceof Error ? error.message : copy.app.unknownError,
      })
      return false
    }
    cleanup()
      ...
      <div style={{ flex: 1, display: 'grid', gridTemplateColumns: 'auto minmax(0, 1fr) auto', minHeight: 0 }}>
        ...
        {state.analysisStatus === 'done' && (
          <RallyQueue
            ...
            onOpenExportFile={(outputPath) => window.api.openExportPath(outputPath)}
