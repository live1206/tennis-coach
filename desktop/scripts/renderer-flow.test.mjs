...
const canonicalAnalysis = (segments) => ({
  schema: { name: 'tennis-coach-analysis', version: 1 },
  data_quality: { warnings: [] },
  analysis_capabilities: { supported: [], unsupported: [] },
  players: {},
  segments,
})
...
const firstVideoRallies = createRalliesForVideo(batchVideos[0], canonicalAnalysis([
  {
    index: 2,
    start: 30,
    end: 38,
    motion: { player_motion_max: 0.018 },
    audio: { hit_count: 7 },
    players: {
      player_1: { detected: true, side: 'far', trajectory_samples: 5 },
      player_2: { detected: true, side: 'near', trajectory_samples: 5 },
    },
  },
  { index: 1, start: 10, end: 22, motion: { player_motion_max: 0.025 }, audio: { hit_count: 16 } },
]))
const secondVideoRallies = createRalliesForVideo(batchVideos[1], canonicalAnalysis([
  { index: 0, start: 5, end: 12, motion: { player_motion_max: 0.021 } },
]))
...
assert.equal(LANGUAGE_STORAGE_KEY, 'tennis-coach-desktop-language')
...
assert.equal(hasReusableAnalysisReport(canonicalAnalysis([{ index: 1, start: 10, end: 20 }])), true)
assert.equal(hasReusableAnalysisReport({
  ...canonicalAnalysis([{ index: 1 }]),
  schema: { name: 'other-analysis', version: 1 },
}), false)
assert.equal(hasReusableAnalysisReport(canonicalAnalysis([])), false)
assert.equal(validateTennisAnalysis(canonicalAnalysis([{ index: 1, start: 10, end: 20 }])).schema.name, 'tennis-coach-analysis')
assert.throws(
  () => validateTennisAnalysis({ ...canonicalAnalysis([]), data_quality: { warnings: 'low confidence' } }),
  /data_quality\.warnings/,
)
assert.throws(
  () => validateTennisAnalysis(canonicalAnalysis([{ index: 1, start: 20, end: 10 }])),
  /increasing start\/end/,
)
assert.throws(
  () => validateTennisAnalysis(canonicalAnalysis([
...
const aiBridgeSource = fs.readFileSync(path.join(root, 'src', 'main', 'aiAnalysisBridge.ts'), 'utf8')
const processControlSource = fs.readFileSync(path.join(root, 'src', 'main', 'processControl.ts'), 'utf8')
const ffmpegSource = fs.readFileSync(path.join(root, 'src', 'main', 'ffmpegBridge.ts'), 'utf8')
const engineEntrySource = fs.readFileSync(path.join(root, '..', 'TennisCoachAnalysis.py'), 'utf8')
...
const aiScreenSource = fs.readFileSync(path.join(root, 'src', 'renderer', 'components', 'AIAnalysisScreen.tsx'), 'utf8')
...
assert.match(mainSource, /ipcMain\.handle\('load-video-analysis'/)
assert.match(mainSource, /approvedVideoPaths\.has\(resolvedVideoPath\)/)
assert.match(mainSource, /loadCachedAnalysisReport\(resolvedVideoPath\)/)
assert.match(mainSource, /createHash\('sha256'\)/)
assert.doesNotMatch(mainSource, /approvedAnalysisPaths/)
assert.doesNotMatch(mainSource, /open-analysis-dialog|load-analysis-json/)
assert.match(mainSource, /validateTennisAnalysis\(parsed\)/)
assert.match(aiBridgeSource, /ipcMain\.handle\(\s*'run-local-ai-analysis'/)
assert.match(aiBridgeSource, /resolveAnalysisPath\(videoPath\)/)
assert.match(aiBridgeSource, /currentEvidenceId !== evidenceId/)
assert.match(aiBridgeSource, /tennis-coach-ai-\$\{randomUUID\(\)\}\.json/)
assert.match(aiBridgeSource, /fs\.rmSync\(snapshotPath, \{ force: true \}\)/)
assert.match(aiBridgeSource, /ipcMain\.handle\('cancel-local-ai-analysis'/)
assert.match(aiBridgeSource, /'-m',\s*'video_extraction\.local_analysis'/)
assert.doesNotMatch(aiBridgeSource, /shell:\s*true/)
assert.match(preloadSource, /loadVideoAnalysis/)
assert.match(preloadSource, /runLocalAIAnalysis/)
assert.match(preloadSource, /cancelLocalAIAnalysis/)
assert.match(typesSource, /loadVideoAnalysis/)
assert.doesNotMatch(preloadSource, /openAnalysisDialog|loadAnalysisJson/)
assert.doesNotMatch(welcomeSource, /aiAnalysisTitle|onOpenAIAnalysis/)
assert.doesNotMatch(aiScreenSource, /copy\.aiAnalysis\.supported/)
assert.doesNotMatch(aiScreenSource, /copy\.aiAnalysis\.qualityWarnings/)
assert.doesNotMatch(aiScreenSource, /copy\.aiAnalysis\.unsupported/)
assert.match(aiScreenSource, /copy\.aiAnalysis\.actualResults/)
assert.match(aiScreenSource, /analysis\.data_quality\.audio\?\.hit_count/)
assert.match(aiScreenSource, /analysis\.data_quality\.shots\?\.classified_count/)
...
assert.match(aiScreenSource, /useEffect\(\(\) =>/)
assert.match(aiScreenSource, /startedRef\.current/)
assert.doesNotMatch(aiScreenSource, /onClick=\{runAnalysis\}/)
assert.match(aiScreenSource, /copy\.aiAnalysis\.askTitleCloud/)
assert.match(aiBridgeSource, /const pipelineScript = path\.join\(getProjectRoot\(\), 'run_coach_pipeline\.py'\)/)
assert.match(aiScreenSource, /copy\.aiAnalysis\.privacyCloud/)
assert.doesNotMatch(aiScreenSource, /provider === 'cloud'/)
assert.doesNotMatch(aiScreenSource, /\{loaded\.path\}/)
assert.match(ffmpegSource, /interface ExportClip/)
assert.match(ffmpegSource, /videoPath: string/)
assert.match(ffmpegExecutableSource, /ipcMain\.handle\('export-highlights', async \(event, clips: unknown\) =>/)
assert.match(ffmpegExecutableSource, /clips\.some\(\(clip\) => !isApprovedVideoPath\(clip\.videoPath\)\)/)
assert.match(ffmpegExecutableSource, /ipcMain\.handle\('open-export-path'/)
assert.match(ffmpegExecutableSource, /completedExportPaths\.has\(resolved\)/)
...
assert.match(ffmpegExecutableSource, /currentExport\.cancelled \|\| signal === 'SIGTERM'/)
assert.match(ffmpegExecutableSource, /completedExportPaths\.add\(outputPath\)/)
...
assert.match(preloadSource, /ipcRenderer\.invoke\('approve-dropped-video-paths', videoPaths\)/)
assert.doesNotMatch(preloadSource, /approveVideoPaths/)
assert.match(preloadSource, /openExportPath/)
...
assert.match(typesSource, /resolveDroppedVideoPaths: \(files: File\[\]\) => Promise<string\[\]>/)
...
assert.match(mainSource, /url === productionRendererUrl/)
assert.doesNotMatch(mainSource, /url\.startsWith\('file:'\)/)
assert.match(mainSource, /const VITE_DEV_SERVER_URL = app\.isPackaged \? undefined : process\.env\.VITE_DEV_SERVER_URL/)
assert.match(pythonBridgeExecutableSource, /sourceMetadata = getSourceMetadata\(videoPath\)[\s\S]*spawn\(cmd/)
assert.match(pythonBridgeExecutableSource, /sourceMetadataMatches\(sourceMetadata, currentMetadata\)/)
...
assert.match(appSource, /window\.api\.loadVideoAnalysis\(videoPath\)/)
assert.match(appSource, /activeVideo\?\.status === 'done'/)
assert.match(appSource, /handleOpenAIAnalysis\(activeVideo\.path\)/)
assert.match(appSource, /copy\.aiAnalysis\.launch/)
assert.match(appSource, /&& !hasCompletedVideo/)
assert.match(appSource, /state\.analysisStatus === 'running' && \([\s\S]*cancelBatchAnalysis/)
...
assert.match(engineEntrySource, /from video_extraction\.cli import main/)
assert.match(engineEntrySource, /raise SystemExit\(main\(\)\)/)
...
assert.match(pythonBridgeSource, /terminateProcessTree\(processToCancel\)/)
assert.match(aiBridgeSource, /terminateProcessTree\(localAnalysisProcess\)/)
assert.match(ffmpegSource, /terminateProcessTree\(currentExport\.process\)/)
assert.match(processControlSource, /process\.kill\(-child\.pid/)
assert.match(processControlSource, /'taskkill'/)
