...
assert.match(aiScreenSource, /copy\.aiAnalysis\.qualityWarnings/)
assert.match(aiScreenSource, /window\.api\.runLocalAIAnalysis/)
assert.match(aiScreenSource, /window\.api\.cancelAIAnalysis/)
assert.match(aiScreenSource, /useState\('qwen2\.5-0\.5b'\)/)
assert.match(aiScreenSource, /disabled=\{running\}/)
assert.match(aiBridgeSource, /apiPath\.replace\(\/\^\\\/\+\/, ''\)/)
assert.match(aiScreenSource, /provider === 'cloud' \? copy\.aiAnalysis\.privacyCloud : copy\.aiAnalysis\.privacyLocal/)
