...
ENGINE_EXE = PROJECT_ROOT / "dist-engine" / "TennisCoachAnalysis" / "TennisCoachAnalysis.exe"
LOCAL_ANALYSIS_EXE = PROJECT_ROOT / "dist-engine" / "TennisCoachLocalAnalysis" / "TennisCoachLocalAnalysis.exe"
FFMPEG_EXE = PROJECT_ROOT / "dist-engine" / "ffmpeg" / "ffmpeg.exe"
DEFAULT_WINDOWS_OUTPUT = Path("C:/tennis-coach-msi")
...
def build_engine() -> None:
    ...
    require_file(
        LOCAL_ANALYSIS_EXE,
        "Local AI build completed, but the packaged executable was not found.",
    )
    require_file(
        FFMPEG_EXE,
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Build the Tennis Coach desktop MSI installer.")
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    if args.skip_engine:
        ...
        require_file(LOCAL_ANALYSIS_EXE, "Cannot skip engine build because the local AI executable is missing.")
